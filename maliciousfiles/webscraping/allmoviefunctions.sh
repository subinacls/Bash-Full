...
#
# To make these permanent within the users terminal use the following command:
#    cat allmoviefunctions.sh >> ~/.bashrc
#
...
randomUA() {
    chkshuf() {
      ...
      if [ -z $whichshuf ]; 
        then 
          echo "[*] The application Shuffle is not installed on your system" 1>&2; 
          echo "    [-] Please use: sudo apt-get -y install shuf to use this script" 1>&2;
          echo "       [-] Would you like to install shuf on your system" 1>&2;
          read answer;
          case ${answer:0:1} in
            y|Y )
              sudo apt-get install -y shuf
            ;;
            * )
              echo ""
            ;;
...
Opera/9.99 (X11; U; sk)\n"""
  if [ ! -f newarray ]; 
    then
      echo -e $content | sed -r "s/^ (.*)/\1/g" > newarray;
      newUA=$(shuf -n 1 newarray);
      echo $newUA;
    else
      newUA=$(shuf -n 1 newarray);
      echo $newUA;
...
################################################################################
# Needed to generate the newarray of UserAgent string on disk
...
getinfo () {
    ...
    echo;
    read -p "      [?] Would you like to download this movie/show (y/n)? " answer;
      case ${answer:0:1} in
        y|Y )
          downthemovie $durl
        ;;
...
listepisodes() {
    ...
    sort -u -V -t "/" -k6;
    echo;
    read -p "      [?] Would you like to download the season (y/n)? " answer;
      case ${answer:0:1} in
        y|Y )
          downtheseason $durl
        ;;
